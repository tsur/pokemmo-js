...
class ClientCharacter {
	...
	private function onWalk():Void {
		...
		if (destSolid == GameMap.SD_GRASS && mapInstance.map.grassEncounters != null) {
	...
	private inline function canWalkOnTileType(type:Int):Bool {
		return surfing ? (type == GameMap.SD_WATER) : (type == GameMap.SD_NONE || type == GameMap.SD_GRASS);
	}
