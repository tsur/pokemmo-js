...
class CCharacter extends GameObject {
	...
	public function getRenderPosX():Int {
		if (!walking) return Math.floor(x * GameMap.cur.tilewidth + renderOffsetX);
		
		var destX:Float = x * GameMap.cur.tilewidth;
		var perc = (walkingPerc - CHAR_MOVE_WAIT) * (1.0/(1.0-CHAR_MOVE_WAIT));
		if(walkingPerc >= CHAR_MOVE_WAIT){
			if(walkingHasMoved){
				switch(direction){
					case Game.DIR_LEFT: destX += (GameMap.cur.tilewidth) * (1-perc);
					case Game.DIR_RIGHT: destX -= (GameMap.cur.tilewidth) * (1-perc);
				}
			}else{
				switch(direction){
					case Game.DIR_LEFT: destX -= (GameMap.cur.tilewidth) * perc;
					case Game.DIR_RIGHT: destX += (GameMap.cur.tilewidth) * perc;
				}
			}
		}
	...
	public function getRenderPosY():Int {
		if (!walking) return Math.floor(y * GameMap.cur.tileheight - CHAR_HEIGHT / 2 + renderOffsetY);
		...
		if(walkingPerc >= CHAR_MOVE_WAIT){
			if(walkingHasMoved){
				switch(direction){
					case Game.DIR_UP: destY += (GameMap.cur.tileheight) * (1-perc);
					case Game.DIR_DOWN: destY -= (GameMap.cur.tileheight) * (1-perc);
				}
			}else{
				switch(direction){
					case Game.DIR_UP: destY -= (GameMap.cur.tileheight) * perc;
					case Game.DIR_DOWN: destY += (GameMap.cur.tileheight) * perc;
				}
			}
		}
			...
			if (walkingPerc >= (1.0 - CHAR_MOVE_WAIT) / 2 && !walkingHasMoved) {
				...
				if(isControllable() && !noclip){
					...
					if(tmpWarp != null && tmpWarp.canWarp(this)){
						...
						if (GameMap.cur.isTileLedge(frontX, frontY) && GameMap.cur.getLedgeDir(frontX, frontY) == direction) {
							useLedge();
